...
public class AuthenticationController {
    ...
    public ResponseEntity<AuthResponse> authenticate(@Valid @RequestBody AuthRequest request, HttpSession session) {
        return authenticationService.findUserByEmail(request.username())
                .filter(user -> authenticationService.passwordMatches(request.password(), user.passwordHash()))
                .map(user -> {
                ...
                .orElseGet(() -> ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new AuthResponse(false)));
    }
